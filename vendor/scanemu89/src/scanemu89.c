...
static scanemu_i32 se_abs_q16(scanemu_i32 x) { return (x < 0) ? -x : x; }
...
static scanemu_bool se_capture_axis(scanemu_ctx* ctx, const scanemu_event* ev, scanemu_token* out_binding)
{
    ...
    v = ev->token.value_q16;
    if (se_abs_q16(v) < ctx->axis_threshold_q16) return SCANEMU_FALSE;

    *out_binding = ev->token;
    if (v >= 0) out_binding->type = SCANEMU_T_PAD_AXIS_POS;
    else           out_binding->type = SCANEMU_T_PAD_AXIS_NEG;

    out_binding->value_q16 = ctx->axis_threshold_q16; /* store threshold */
    return SCANEMU_TRUE;
}
...
void scanemu89_init(scanemu_ctx* ctx, scanemu_binding* storage, scanemu_i32 storage_count)
{
    scanemu_i32 i;
    if (!ctx) return;
    ...
    ctx->axis_threshold_q16 = SCANEMU89_FIXED_HALF;
...
void scanemu89_set_on_bound(scanemu_ctx* ctx, scanemu_on_bound_fn fn, void* user)
{
    if (!ctx) return;
    ctx->on_bound = fn;
...
scanemu_bool scanemu89_is_listening(const scanemu_ctx* ctx)
{
    if (!ctx) return SCANEMU_FALSE;
    return ctx->listening;
}

void scanemu89_listen(scanemu_ctx* ctx, const char* symbol, scanemu_u32 listen_flags)
{
...
void scanemu89_cancel(scanemu_ctx* ctx)
{
    if (!ctx) return;
    ctx->listening = SCANEMU_FALSE;
...
scanemu89_feed_result scanemu89_feed(scanemu_ctx* ctx, const scanemu_event* ev)
{
    scanemu_i32 idx;
    ...
    /* Axis capture from MOVE events: callers should emit token.code=axis_id and token.value_q16 in [-1..1]. */
...
scanemu_bool scanemu89_has(const scanemu_ctx* ctx, const char* symbol)
{
    return (se_find_index(ctx, symbol) >= 0) ? SCANEMU_TRUE : SCANEMU_FALSE;
}

scanemu_bool scanemu89_get(const scanemu_ctx* ctx, const char* symbol, scanemu_token* out_token)
{
    scanemu_i32 idx;
...
scanemu_bool scanemu89_set(scanemu_ctx* ctx, const char* symbol, const scanemu_token* token)
{
    scanemu_i32 idx;
...
scanemu_bool scanemu89_clear(scanemu_ctx* ctx, const char* symbol)
{
    scanemu_i32 idx;
...
scanemu_i32 scanemu89_binding_count(const scanemu_ctx* ctx)
{
    if (!ctx) return 0;
    return ctx->count;
}

scanemu_bool scanemu89_binding_at(const scanemu_ctx* ctx, scanemu_i32 index,
...
scanemu_bool scanemu89_last_get(const scanemu_ctx* ctx, const char** out_symbol, scanemu_token* out_token)
{
...
void scanemu89_last_clear(scanemu_ctx* ctx)
{
    if (!ctx) return;
    ctx->last_valid = SCANEMU_FALSE;
    ctx->last_symbol[0] = '\0';
...
scanemu_bool scanemu89_token_equal(const scanemu_token* a, const scanemu_token* b)
{
    if (!a || !b) return SCANEMU_FALSE;
    ...
    if (a->type == SCANEMU_T_PAD_AXIS_POS || a->type == SCANEMU_T_PAD_AXIS_NEG) {
        scanemu_i32 dv = a->value_q16 - b->value_q16;
        if (dv < 0) dv = -dv;
        if (dv > 1) return SCANEMU_FALSE;
    }
    return SCANEMU_TRUE;
}
...
static scanemu_i32 se_append_q16_2(char* out, scanemu_i32 cap, scanemu_i32* io_len, scanemu_i32 q16)
{
    scanemu_i32 whole;
    scanemu_i32 frac;
    scanemu_i32 neg;
    ...
    neg = q16 < 0;
    absq = neg ? -q16 : q16;
    whole = absq / SCANEMU89_FIXED_ONE;
    frac = ((absq % SCANEMU89_FIXED_ONE) * 100L + (SCANEMU89_FIXED_ONE / 2L)) /
           SCANEMU89_FIXED_ONE;
    if (frac >= 100) { whole += 1; frac -= 100; }
    if (neg && !se_append_char(out, cap, io_len, '-')) return 0;
    if (!se_append_int(out, cap, io_len, whole)) return 0;
    if (!se_append_char(out, cap, io_len, '.')) return 0;
    if (frac < 10 && !se_append_char(out, cap, io_len, '0')) return 0;
    return se_append_int(out, cap, io_len, frac);
}
...
static scanemu_bool se_parse_q16(const char** io_s, scanemu_i32* out_q16)
{
    const char* s;
    scanemu_i32 sign;
    scanemu_i32 whole;
    scanemu_i32 frac;
    scanemu_i32 frac_div;
    scanemu_i32 digits;
    if (!io_s || !*io_s || !out_q16) return SCANEMU_FALSE;
    s = se_skip_ws(*io_s);
    sign = 1;
    if (*s == '-') { sign = -1; s++; }
    whole = 0;
    if (*s < '0' || *s > '9') return SCANEMU_FALSE;
    while (*s >= '0' && *s <= '9') {
        whole = whole * 10 + (scanemu_i32)(*s - '0');
        s++;
    }
    frac = 0;
    frac_div = 1;
    ...
    *out_q16 = sign * (whole * SCANEMU89_FIXED_ONE +
                       (frac * SCANEMU89_FIXED_ONE + frac_div / 2) / frac_div);
    *io_s = s;
    return SCANEMU_TRUE;
}

scanemu_bool scanemu89_token_to_string(const scanemu_token* t, char* out, scanemu_i32 out_cap)
{
    scanemu_i32 len = 0;
    ...
    if (t->type == SCANEMU_T_PAD_AXIS_POS || t->type == SCANEMU_T_PAD_AXIS_NEG) {
        if (!se_append_char(out, out_cap, &len, ':')) return SCANEMU_FALSE;
        if (!se_append_q16_2(out, out_cap, &len, t->value_q16)) return SCANEMU_FALSE;
...
scanemu_bool scanemu89_token_from_string(scanemu_token* out_t, const char* s)
{
    const char* p;
    scanemu_i32 device;
    scanemu_i32 code;
    ...
    if (out_t->type == SCANEMU_T_PAD_AXIS_POS || out_t->type == SCANEMU_T_PAD_AXIS_NEG) {
        if (*p != ':') return SCANEMU_FALSE;
        p++;
        if (!se_parse_q16(&p, &val)) return SCANEMU_FALSE;
        out_t->value_q16 = val;
...
scanemu_bool scanemu89_binding_matches_event(const scanemu_token* binding, const scanemu_event* ev)
{
    ...
    if (binding->type == SCANEMU_T_KEY ||
        ...
        return scanemu89_token_equal(binding, &ev->token);
    }
    ...
    if (binding->type == SCANEMU_T_PAD_AXIS_POS || binding->type == SCANEMU_T_PAD_AXIS_NEG) {
        ...
        if (binding->type == SCANEMU_T_PAD_AXIS_POS) {
            if (v < 0) return SCANEMU_FALSE;
            return (v >= binding->value_q16) ? SCANEMU_TRUE : SCANEMU_FALSE;
        } else {
            if (v > 0) return SCANEMU_FALSE;
            return ((-v) >= binding->value_q16) ? SCANEMU_TRUE : SCANEMU_FALSE;
        }
