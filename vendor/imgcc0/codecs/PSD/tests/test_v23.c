...
static psd89_u32 idx5(psd89_u32 x, psd89_u32 y)
{
    return y * 5U + x;
}

static int test_drop_shadow_and_outer_glow_2d(void)
{
    static const char norm[4] = { 'n', 'o', 'r', 'm' };
    static const unsigned char r[25] = {
        0U,0U,0U,0U,0U,
        0U,0U,0U,0U,0U,
        0U,0U,255U,0U,0U,
        0U,0U,0U,0U,0U,
    ...
    static const unsigned char g[25] = {
        0U,0U,0U,0U,0U,
        0U,0U,0U,0U,0U,
        0U,0U,255U,0U,0U,
        0U,0U,0U,0U,0U,
    ...
    static const unsigned char a[25] = {
        0U,0U,0U,0U,0U,
        0U,0U,0U,0U,0U,
        0U,0U,255U,0U,0U,
        0U,0U,0U,0U,0U,
    ...
    unsigned char out_r[25];
    unsigned char out_g[25];
    unsigned char out_b[25];
    unsigned char out_a[25];
    psd89_doc doc;

    init_doc(&doc, 5U, 5U);
    init_rgb_layer_rect(&doc.layers[0], 0, 0, 5, 5, "lrfx2d", norm, r, g, b, a, 5U);
    ...
    doc.layers[0].lrfx.drop_shadow.blur = 2;
    ...
    if (!expect_true(out_a[idx5(3, 2)] > out_a[idx5(4, 2)], "drop main stronger than tail")) {
        return 0;
    }
    if (!expect_true(out_a[idx5(3, 1)] > 0U && out_a[idx5(3, 3)] > 0U, "drop has vertical falloff")) {
        return 0;
    }
    if (!expect_true(out_r[idx5(3, 1)] == 0U && out_g[idx5(3, 1)] == 0U && out_b[idx5(3, 1)] == 0U, "drop shadow keeps shadow color")) {
        return 0;
    }
    ...
    doc.layers[0].lrfx.outer_glow.blur = 2;
    ...
    if (!expect_true(out_a[idx5(1, 1)] > 0U, "outer glow reaches diagonal")) {
        return 0;
    }
    if (!expect_true(out_r[idx5(1, 1)] >= out_g[idx5(1, 1)] && out_g[idx5(1, 1)] > 0U, "outer glow writes color")) {
        return 0;
    }
    return expect_true(out_a[idx5(2, 2)] == 255U, "outer glow keeps center opaque");
}

static int test_inner_glow_and_bevel_2d(void)
{
    static const char norm[4] = { 'n', 'o', 'r', 'm' };
    static const unsigned char r[25] = {
    ...
    init_doc(&doc, 5U, 5U);
    init_rgb_layer_rect(&doc.layers[0], 0, 0, 5, 5, "inner2d", norm, r, g, b, a, 5U);
    ...
    doc.layers[0].lrfx.inner_glow.blur = 2;
    ...
    if (!expect_true(out_g[idx5(0, 0)] > out_g[idx5(2, 2)], "inner glow stronger at edge than center")) {
        return 0;
    }
    ...
    doc.layers[0].lrfx.bevel.angle = 315;
    doc.layers[0].lrfx.bevel.blur = 2;
    doc.layers[0].lrfx.bevel.strength = 100;
    ...
    if (!expect_true((unsigned int)out_r[idx5(0, 0)] + (unsigned int)out_g[idx5(0, 0)] + (unsigned int)out_b[idx5(0, 0)] >
                     (unsigned int)out_r[idx5(2, 2)] + (unsigned int)out_g[idx5(2, 2)] + (unsigned int)out_b[idx5(2, 2)],
                     "bevel highlights one corner")) {
        return 0;
    }
    return expect_true(out_r[idx5(4, 4)] < out_r[idx5(2, 2)], "bevel darkens opposite corner");
}

int main(void)
{
    if (!test_drop_shadow_and_outer_glow_2d()) {
        return 1;
    }
    if (!test_inner_glow_and_bevel_2d()) {
        return 1;
    }
    printf("test_v23: ok\n");
    return 0;
