...
```sh
sh ./scripts/repro_crash.sh libfuzzer ./tests/fuzz_pcx_libfuzzer path/to/input
sh ./scripts/repro_crash.sh file ./tests/fuzz_pcx_stdin path/to/input
```


## Round 5: CI + split harnesses

This round adds mode-focused fuzz entry points and GitHub Actions smoke coverage:

- `tests/fuzz_libfuzzer_inspect.c`
- `tests/fuzz_libfuzzer_rgb.c`
- `tests/fuzz_libfuzzer_indexed.c`
- `tests/fuzz_stdin_inspect.c`
- `tests/fuzz_stdin_rgb.c`
- `tests/fuzz_stdin_indexed.c`
...
- sanitizer smoke coverage for a standalone C smoke test (`tests/test_smoke.c`)
- per-target smoke jobs for `inspect`, `rgb`, and `indexed`
...
```sh
make test-smoke
make test-smoke TEST_SANITIZERS=address,undefined TEST_CFLAGS='-O1 -g -fno-omit-frame-pointer'
make seedcheck-libfuzzer-inspect FUZZ_RUNS=64 FUZZ_SANITIZERS=address,undefined
make seedcheck-stdin-rgb
make corpus-min-libfuzzer-indexed FUZZ_SANITIZERS=address,undefined
make bundle-ci-inspect
```
