...
- **GCC**: keep the baseline `-Wall -Wextra -Werror -pedantic` and add contract-oriented checks such as `-Wstrict-prototypes`, `-Wmissing-prototypes`, `-Wshadow`, `-Wpointer-arith`, `-Wformat=2`, `-Wundef`, `-Walloc-zero`, and `-Wmismatched-dealloc`.
