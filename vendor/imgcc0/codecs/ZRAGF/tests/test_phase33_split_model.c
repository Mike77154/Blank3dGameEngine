...
int main(void)
{
    ...
    if (!(split_size + 64u < single_size)) {
        fprintf(stderr, "expected stronger mixed split gain: single=%lu split=%lu\n",
                (unsigned long)single_size, (unsigned long)split_size);
        return 1;
    }
    if (!(split_size < zlib_size)) {
        fprintf(stderr, "expected zragf mixed output to beat zlib: split=%lu zlib=%lu\n",
                (unsigned long)split_size, (unsigned long)zlib_size);
        return 1;
    }

    printf("phase33 split-model single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
