...
int main(void)
{
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase62 split encode failed\n");
        return 1;
    }
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase62 split re-encode failed\n");
        return 1;
    }

    if (split_size_again != split_size) {
        fprintf(stderr, "phase62 split re-encode size mismatch: first=%lu second=%lu\n",
    ...
    if (!(split_size <= 65698u)) {
        fprintf(stderr, "expected phase62 to preserve phase62 ratio target: split=%lu\n",
                (unsigned long)split_size);
        return 1;
    }

    printf("phase62 miss-paths single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
