...
int main(void)
{
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase54 split encode failed\n");
        return 1;
    }
    ...
    if (!(split_size <= 65698u)) {
        fprintf(stderr, "expected phase54 to preserve phase54 ratio target: split=%lu\n",
                (unsigned long)split_size);
        return 1;
    }

    printf("phase54 exact-tail-cache single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
