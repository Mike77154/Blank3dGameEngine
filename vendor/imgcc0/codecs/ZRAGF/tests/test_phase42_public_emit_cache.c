...
int main(void)
{
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_single_with_dict(NULL, 0u,
                                                               ...
                                                               0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase42 single encode failed\n");
        return 1;
    }
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase42 split encode failed\n");
        return 1;
    }

    if (!verify_raw_zlib(single_out, single_size, src, n) ||
        !verify_raw_zlib(split_out, split_size, src, n)) {
        fprintf(stderr, "phase42 external zlib verification failed\n");
        return 1;
    }

    zlib_size = zlib_raw_size(src, n);
    if (zlib_size == 0u) {
        fprintf(stderr, "phase42 zlib encode failed\n");
        return 1;
    }

    if (!(split_size + 100u < single_size)) {
        fprintf(stderr, "expected phase42 split path to stay clearly better than single: single=%lu split=%lu\n",
                (unsigned long)single_size, (unsigned long)split_size);
        return 1;
    }
    if (!(split_size < zlib_size)) {
        fprintf(stderr, "expected phase42 split path to beat zlib on mixed corpus: split=%lu zlib=%lu\n",
                (unsigned long)split_size, (unsigned long)zlib_size);
        return 1;
    }
    if (!(split_size <= 65698u)) {
        fprintf(stderr, "expected phase42 to preserve phase41 ratio target: split=%lu\n",
                (unsigned long)split_size);
        return 1;
    }

    printf("phase42 public emit-cache single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
