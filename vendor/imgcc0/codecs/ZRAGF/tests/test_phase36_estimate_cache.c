...
int main(void)
{
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "phase36 split encode failed\n");
        return 1;
    }
    ...
    if (!(split_size + 100u < single_size)) {
        fprintf(stderr, "expected cached split path to stay clearly better than single: single=%lu split=%lu\n",
                (unsigned long)single_size, (unsigned long)split_size);
        return 1;
    }
    if (!(split_size < zlib_size)) {
        fprintf(stderr, "expected cached split path to beat zlib on mixed corpus: split=%lu zlib=%lu\n",
                (unsigned long)split_size, (unsigned long)zlib_size);
        return 1;
    }
    if (!(split_size <= 65720u)) {
        fprintf(stderr, "expected phase36 to keep phase35-class ratio: split=%lu\n",
                (unsigned long)split_size);
        return 1;
    }

    printf("phase36 estimate-cache single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
