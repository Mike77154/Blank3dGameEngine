...
int main(void)
{
    ...
    if (!zragf_deflate_rfc1951_compress_chunk_with_dict(NULL, 0u,
                                                        ...
                                                        ZRAGF_Z_DEFAULT_STRATEGY,
                                                        0, 0, 0, 0, 0)) {
        fprintf(stderr, "budgeted encode failed\n");
        return 1;
    }
    ...
    if (!(split_size + 100u < single_size)) {
        fprintf(stderr, "expected budgeted path to stay clearly better than single: single=%lu split=%lu\n",
                (unsigned long)single_size, (unsigned long)split_size);
        return 1;
    }
    if (!(split_size < zlib_size)) {
        fprintf(stderr, "expected budgeted path to beat zlib on mixed corpus: split=%lu zlib=%lu\n",
                (unsigned long)split_size, (unsigned long)zlib_size);
        return 1;
    }
    if (!(split_size <= 65720u)) {
        fprintf(stderr, "expected phase35 to keep near-phase33 ratio: split=%lu\n",
                (unsigned long)split_size);
        return 1;
    }

    printf("phase35 partition budget single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld\n",
