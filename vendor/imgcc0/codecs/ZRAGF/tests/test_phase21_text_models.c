...
static size_t zragf_filtered_raw_size(const unsigned char *src, size_t src_len)
{
    zragf_stream zs;
    unsigned char *out;
    size_t cap = src_len * 2u + 1024u;
    ...
    assert(out != NULL);
    memset(&zs, 0, sizeof(zs));
    rc = zragf_deflateInit2(&zs, 6, 8, -15, 8, ZRAGF_Z_FILTERED);
    assert(rc == ZRAGF_OK);
    zs.next_in = (unsigned char *)src;
    zs.avail_in = src_len;
    zs.next_out = out;
...
int main(void)
{
    ...
    size_t json_auto = zragf_raw_size(jsonish, json_len);
    size_t json_filtered = zragf_filtered_raw_size(jsonish, json_len);
    size_t json_zlib = zlib_raw_size(jsonish, json_len);
    size_t log_auto = zragf_raw_size(logish, log_len);
    size_t log_filtered = zragf_filtered_raw_size(logish, log_len);
    size_t log_zlib = zlib_raw_size(logish, log_len);

    assert(json_auto == json_filtered);
    assert(log_auto == log_filtered);
    assert(json_auto <= json_zlib);
    assert(log_auto <= log_zlib);

    printf("phase21 text models ok json=%lu/%lu log=%lu/%lu\n",
           (unsigned long)json_auto, (unsigned long)json_zlib,
           (unsigned long)log_auto, (unsigned long)log_zlib);
