...
def main() -> int:
    ap = argparse.ArgumentParser(description='Run the phase 69 public benchmark suite, lock corpora by hash, and emit PNG/TIFF differential validation results.')
    ap.add_argument('--exe', default='build/zragf_bench_phase69_public')
    ap.add_argument('--out-dir', default='bench/results/phase69')
    ...
    frozen_manifest_path = out_dir / 'phase69_frozen_manifest.json'
    csv_path = out_dir / 'phase69_public.csv'
    summary_path = out_dir / 'phase69_public.md'
    validation_json = out_dir / 'phase69_png_tiff_validation.json'
    metadata_json = out_dir / 'phase69_run_metadata.json'

    fetch_cmd = [
        sys.executable,
        'tools/fetch_phase69_corpora.py',
    ...
    validate_cmd = [
        sys.executable,
        'tools/validate_phase69_png_tiff.py',
        '--manifest', str(manifest_path),
        '--root', args.corpus_root,
