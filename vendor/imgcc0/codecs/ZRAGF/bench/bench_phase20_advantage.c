...
static unsigned char *make_case(const char *name, size_t *out_size)
{
    ...
    if (strcmp(name, "ramp") == 0) {
        *out_size = 131072u;
        buf = (unsigned char *)zragf_p89_host_take(*out_size);
        if (!buf) return NULL;
        for (i = 0u; i < *out_size; ++i) buf[i] = (unsigned char)(i & 0xFFu);
        return buf;
    }
...
int main(void)
{
    static const char *cases[] = {"mixed", "ramp", "jsonish"};
    size_t i;
    for (i = 0u; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        size_t n = 0u;
        size_t out_zragf = 0u, out_zlib = 0u;
        ...
        printf("phase20 %s raw: zragf=%lu zlib=%lu delta=%ld ms20_zragf=%d ms20_zlib=%d\n",
               cases[i],
