...
int main(void)
{
    ...
    size_t json_zragf = 0u, json_zlib = 0u;
    size_t log_zragf = 0u, log_zlib = 0u;
    zragf_fx json_ms_zragf, json_ms_zlib, log_ms_zragf, log_ms_zlib;
    unsigned char *jsonish = make_jsonish(&json_n);
    unsigned char *logish = make_logish(&log_n);
    if (!jsonish || !logish) return 1;

    json_ms_zragf = bench_ms(zragf_size_once, jsonish, json_n, 20, &json_zragf);
    json_ms_zlib = bench_ms(zlib_size_once, jsonish, json_n, 20, &json_zlib);
    log_ms_zragf = bench_ms(zragf_size_once, logish, log_n, 20, &log_zragf);
    log_ms_zlib = bench_ms(zlib_size_once, logish, log_n, 20, &log_zlib);

    printf("phase22 jsonish raw: zragf=%lu zlib=%lu delta=%ld ms_zragf=%d ms_zlib=%d\n",
           (unsigned long)json_zragf, (unsigned long)json_zlib,
           (long)json_zlib - (long)json_zragf, json_ms_zragf, json_ms_zlib);
    printf("phase22 logish raw:  zragf=%lu zlib=%lu delta=%ld ms_zragf=%d ms_zlib=%d\n",
           (unsigned long)log_zragf, (unsigned long)log_zlib,
           (long)log_zlib - (long)log_zragf, log_ms_zragf, log_ms_zlib);
