...
static size_t zragf_single_size(const unsigned char *src, size_t n)
{
...
static size_t zragf_budget_size(const unsigned char *src, size_t n)
{
    size_t cap = zragf_deflate_rfc1951_stored_bound(n) + 512u;
    unsigned char *out = (unsigned char *)zragf_p89_host_take(cap);
    size_t out_len = 0u;
...
{
    size_t n = 0u;
    size_t out_single = 0u, out_split = 0u, out_zlib = 0u;
    zragf_fx ms_single, ms_split, ms_zlib;
    unsigned char *buf = make_case(&n);
    if (!buf)
        return 1;
    ms_single = bench_ms(zragf_single_size, buf, n, 10, &out_single);
    ms_split = bench_ms(zragf_budget_size, buf, n, 10, &out_split);
    ms_zlib = bench_ms(zlib_size_once, buf, n, 10, &out_zlib);
    printf("phase35 mixed raw: single=%lu split=%lu zlib=%lu delta_single=%ld delta_zlib=%ld ms_single=%d ms_split=%d ms_zlib=%d\n",
           (unsigned long)out_single,
           (unsigned long)out_split,
           (unsigned long)out_zlib,
           (long)out_single - (long)out_split,
           (long)out_zlib - (long)out_split,
           ms_single, ms_split, ms_zlib);
