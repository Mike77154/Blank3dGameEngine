...
#define ZRAGF_P67_MAX_DATASETS 32
#define ZRAGF_P67_MAX_LEVELS 8
#define ZRAGF_P67_MAX_ROWS 256
#define ZRAGF_P67_MB (1024 * 1024)

typedef struct zragf_p67_dataset_s {
    char name[64];
    char family[32];
    char source_kind[16];
    unsigned char *data;
...
typedef struct zragf_p67_row_s {
    char codec[16];
    char wrapper[8];
    int level;
    char dataset[64];
...
typedef struct zragf_p67_agg_s {
    char codec[16];
    int level;
    size_t total_in;
    size_t total_out;
...
static unsigned long zragf_p67_xorshift32(unsigned long *s)
{
    unsigned long x = *s;
    x ^= x << 13;
    x ^= x >> 17;
...
static void *zragf_p67_malloc(size_t n)
{
    if (n == 0u) n = 1u;
    return zragf_p89_host_take(n);
}

static char *zragf_p67_strdup(const char *s)
{
    size_t n;
    char *d;
    if (!s) return NULL;
...
static const char *zragf_p67_size_class(size_t n)
{
...
static int zragf_p67_add_dataset(zragf_p67_dataset *datasets, int *count,
                                 const char *name, const char *family,
                                 const char *source_kind,
                                 unsigned char *data, size_t size, int owned)
{
    zragf_p67_dataset *ds;
    if (*count >= ZRAGF_P67_MAX_DATASETS) return 0;
    ds = &datasets[*count];
...
static unsigned char *zragf_p67_make_text(size_t n)
{
    ...
    size_t i;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(n);
    if (!buf) return NULL;
    for (i = 0u; i < n; ++i) buf[i] = (unsigned char)chunk[i % chunk_len];
    return buf;
}

static unsigned char *zragf_p67_make_json(size_t n)
{
    size_t i, pos;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(n);
    if (!buf) return NULL;
...
static unsigned char *zragf_p67_make_logs(size_t n)
{
    size_t pos;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(n);
    if (!buf) return NULL;
...
static unsigned char *zragf_p67_make_png_scanlines(size_t rows, size_t cols)
{
    size_t row, col, pos, stride;
    unsigned char *buf;
    stride = 1u + cols * 4u;
    buf = (unsigned char *)zragf_p67_malloc(rows * stride);
    if (!buf) return NULL;
    pos = 0u;
...
static unsigned char *zragf_p67_make_photoish(size_t n)
{
    size_t i;
    unsigned long st;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(n);
    ...
    st = 0x12345678ul;
    for (i = 0u; i < n; ++i) {
        unsigned long noise = zragf_p67_xorshift32(&st) & 31ul;
        buf[i] = (unsigned char)(((i * 7u) + (i / 257u) + noise) & 0xFFu);
    }
    return buf;
}

static unsigned char *zragf_p67_make_normalmap(size_t pixels)
{
    size_t i, pos;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(pixels * 4u);
    if (!buf) return NULL;
    pos = 0u;
    for (i = 0u; i < pixels; ++i) {
...
static unsigned char *zragf_p67_make_binary(size_t n)
{
    size_t i;
    unsigned long st;
    unsigned char *buf = (unsigned char *)zragf_p67_malloc(n);
    if (!buf) return NULL;
    st = 0xCAFEBABEul;
    for (i = 0u; i < n; ++i) buf[i] = (unsigned char)(zragf_p67_xorshift32(&st) & 0xFFu);
    return buf;
}

static int zragf_p67_add_builtins(zragf_p67_dataset *datasets, int *count, int quick)
{
    if (!zragf_p67_add_dataset(datasets, count, "text_small", "text", "synthetic", zragf_p67_make_text(32768u), 32768u, 1)) return 0;
    if (!zragf_p67_add_dataset(datasets, count, "json_medium", "json", "synthetic", zragf_p67_make_json(196608u), 196608u, 1)) return 0;
    if (!zragf_p67_add_dataset(datasets, count, "logs_medium", "logs", "synthetic", zragf_p67_make_logs(262144u), 262144u, 1)) return 0;
    if (quick) return 1;
    if (!zragf_p67_add_dataset(datasets, count, "png_scanlines", "png", "synthetic", zragf_p67_make_png_scanlines(512u, 64u), 512u * (1u + 64u * 4u), 1)) return 0;
    if (!zragf_p67_add_dataset(datasets, count, "photoish_medium", "photo", "synthetic", zragf_p67_make_photoish(262144u), 262144u, 1)) return 0;
    if (!zragf_p67_add_dataset(datasets, count, "normalmap_medium", "normalmap", "synthetic", zragf_p67_make_normalmap(65536u), 65536u * 4u, 1)) return 0;
    if (!zragf_p67_add_dataset(datasets, count, "binary_medium", "binary", "synthetic", zragf_p67_make_binary(262144u), 262144u, 1)) return 0;
    return 1;
}

static const char *zragf_p67_file_family(const char *path)
{
...
static unsigned char *zragf_p67_read_file(const char *path, size_t *size_ret)
{
    FILE *f;
    long sz;
    unsigned char *buf;
    ...
    buf = (unsigned char *)zragf_p67_malloc((size_t)sz);
...
static const char *zragf_p67_basename(const char *path)
{
...
static int zragf_p67_add_files(zragf_p67_dataset *datasets, int *count, int argc, char **argv, int first)
{
    int i;
    for (i = first; i < argc; ++i) {
        size_t n = 0u;
        unsigned char *buf = zragf_p67_read_file(argv[i], &n);
        if (!buf) return 0;
        if (!zragf_p67_add_dataset(datasets, count, zragf_p67_basename(argv[i]),
                                   zragf_p67_file_family(argv[i]), "file", buf, n, 1)) {
...
static zragf_fx zragf_p67_ms(clock_t ticks)
{
    return zragf_fx_millis(ticks);
}

static int zragf_p67_split_levels(const char *arg, int *levels, int *count)
{
    char *copy;
    char *tok;
    char *save;
    int n;
    copy = zragf_p67_strdup(arg);
    if (!copy) return 0;
    n = 0;
    save = copy;
    while (save && *save) {
        ...
        tok = save;
        if (*tok) {
            if (n >= ZRAGF_P67_MAX_LEVELS) { zragf_p89_host_release(copy); return 0; }
            levels[n++] = atoi(tok);
        }
        if (!comma) break;
        save = comma + 1;
...
static int zragf_p67_bench_zragf(const unsigned char *src, size_t n, int level,
                                 int warmup, int repeat, zragf_p67_row *row)
{
    unsigned char *comp;
    ...
    comp = (unsigned char *)zragf_p67_malloc((size_t)cap);
    decomp = (unsigned char *)zragf_p67_malloc((n + 1u) ? (n + 1u) : 1u);
    if (!comp || !decomp) {
    ...
    t1 = clock();
    c_ms = zragf_fx_avg_nonzero(zragf_p67_ms(t1 - t0), repeat);
    ...
    t1 = clock();
    d_ms = zragf_fx_avg_nonzero(zragf_p67_ms(t1 - t0), repeat);
...
static int zragf_p67_bench_zlib(const unsigned char *src, size_t n, int level,
                                int warmup, int repeat, zragf_p67_row *row)
{
    unsigned char *comp;
    unsigned char *decomp;
    ...
    comp = (unsigned char *)zragf_p67_malloc((size_t)cap);
    decomp = (unsigned char *)zragf_p67_malloc(n ? n : 1u);
    if (!comp || !decomp) {
        zragf_p89_host_release(comp); zragf_p89_host_release(decomp);
        strcpy(row->note, "alloc failed");
        return 0;
...
#ifdef ZRAGF_HAVE_LIBDEFLATE_BENCH
static int zragf_p67_bench_libdeflate(const unsigned char *src, size_t n, int level,
                                      int warmup, int repeat, zragf_p67_row *row)
{
    ...
    comp = (unsigned char *)zragf_p67_malloc(cap);
    decomp = (unsigned char *)zragf_p67_malloc(n ? n : 1u);
    if (!comp || !decomp) {
        zragf_p89_host_release(comp); zragf_p89_host_release(decomp);
...
static void zragf_p67_row_init(zragf_p67_row *row, const char *codec, int level,
                               const zragf_p67_dataset *ds)
{
    ...
    strncpy(row->size_class, zragf_p67_size_class(ds->size), sizeof(row->size_class) - 1u);
...
static int zragf_p67_write_csv(const char *path, const zragf_p67_row *rows, int row_count)
{
    FILE *f;
    int i;
    f = fopen(path, "wb");
...
static void zragf_p67_cpu_string(char *buf, size_t n)
{
    FILE *f;
    char line[256];
    if (!buf || n == 0u) return;
...
static void zragf_p67_add_agg(zragf_p67_agg *aggs, int *count, const zragf_p67_row *row)
{
    int i;
    for (i = 0; i < *count; ++i) {
...
static int zragf_p67_write_summary(const char *path, const zragf_p67_row *rows, int row_count,
                                   int repeat, int warmup, int builtins, int file_count)
{
    FILE *f;
    int i;
    zragf_p67_agg aggs[16];
    int agg_count;
    char cpu[128];
    f = fopen(path, "wb");
    if (!f) return 0;
    zragf_p67_cpu_string(cpu, sizeof(cpu));
    memset(aggs, 0, sizeof(aggs));
    agg_count = 0;
    for (i = 0; i < row_count; ++i) {
        if (rows[i].ok) zragf_p67_add_agg(aggs, &agg_count, &rows[i]);
    }
    fprintf(f, "# ZRAGFLIB public benchmark suite (phase 67)\n\n");
...
#endif
    fprintf(f, "- miniz: not wired in this build (hook pending external source)\n");
    ...
    fprintf(f, "\n## Notes\n\n");
    fprintf(f, "- This suite compares one-shot **zlib-wrapper** compression because it is the cleanest common surface across zragflib, zlib and libdeflate.\n");
...
static void zragf_p67_free_datasets(zragf_p67_dataset *datasets, int count)
{
    int i;
    for (i = 0; i < count; ++i) {
...
{
    zragf_p67_dataset datasets[ZRAGF_P67_MAX_DATASETS];
    zragf_p67_row rows[ZRAGF_P67_MAX_ROWS];
    int dataset_count;
    int row_count;
    int levels[ZRAGF_P67_MAX_LEVELS];
    int level_count;
    int repeat;
    int warmup;
    int use_builtins;
    ...
    use_builtins = 1;
    quick = 0;
    csv_path = "phase67_public.csv";
    summary_path = "phase67_public.md";
    file_arg_start = argc;
    ...
    for (i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--csv") == 0 && i + 1 < argc) {
            ...
            if (!zragf_p67_split_levels(argv[++i], levels, &level_count)) {
    ...
    if (use_builtins) {
        if (!zragf_p67_add_builtins(datasets, &dataset_count, quick)) {
            fprintf(stderr, "failed to build builtin corpus\n");
            zragf_p67_free_datasets(datasets, dataset_count);
            return 1;
        }
    }
    if (file_arg_start < argc) {
        if (!zragf_p67_add_files(datasets, &dataset_count, argc, argv, file_arg_start)) {
    ...
    for (i = 0; i < dataset_count; ++i) {
        for (j = 0; j < level_count; ++j) {
            if (row_count >= ZRAGF_P67_MAX_ROWS) break;
            zragf_p67_row_init(&rows[row_count], "zragf", levels[j], &datasets[i]);
            zragf_p67_bench_zragf(datasets[i].data, datasets[i].size, levels[j], warmup, repeat, &rows[row_count]);
            row_count += 1;
            if (row_count >= ZRAGF_P67_MAX_ROWS) break;
            zragf_p67_row_init(&rows[row_count], "zlib", levels[j], &datasets[i]);
            zragf_p67_bench_zlib(datasets[i].data, datasets[i].size, levels[j], warmup, repeat, &rows[row_count]);
            row_count += 1;
#ifdef ZRAGF_HAVE_LIBDEFLATE_BENCH
            if (row_count >= ZRAGF_P67_MAX_ROWS) break;
            zragf_p67_row_init(&rows[row_count], "libdeflate", levels[j], &datasets[i]);
            zragf_p67_bench_libdeflate(datasets[i].data, datasets[i].size, levels[j], warmup, repeat, &rows[row_count]);
            row_count += 1;
#endif
        }
    }

    if (!zragf_p67_write_csv(csv_path, rows, row_count)) {
        fprintf(stderr, "failed to write csv\n");
        zragf_p67_free_datasets(datasets, dataset_count);
        return 1;
    }
    if (!zragf_p67_write_summary(summary_path, rows, row_count, repeat, warmup, use_builtins,
    ...
    printf("phase67 public bench rows=%d csv=%s summary=%s\n", row_count, csv_path, summary_path);
    zragf_p67_free_datasets(datasets, dataset_count);
    return 0;
}
