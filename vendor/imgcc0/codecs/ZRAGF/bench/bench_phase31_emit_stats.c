...
static zragf_fx bench_internal_dynamic(const unsigned char *src,
                                     size_t n,
                                     int prepared_mode,
                                     size_t *last_size)
{
    ...
    if (prepared_mode && !zragf_deflate_prepare_dynamic(&stats, &prep)) {
        zragf_p89_host_release(out);
        zragf_tokens_free(&tb);
        return 0;
    }
    ...
    for (i = 0; i < 200; ++i) {
        ...
        if (prepared_mode) {
            ...
            if (!zragf_deflate_cost_dynamic(tb.data, tb.size, &stats, 1, 0, 1, &cost) ||
                !zragf_deflate_emit_dynamic_block(tb.data, tb.size, &stats,
                                                  out, cap, &emitted,
                                                  1, NULL, NULL, 1)) {
                zragf_p89_host_release(out);
                zragf_tokens_free(&tb);
...
int main(void)
{
    ...
    for (i = 0u; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        printf("phase31 %s raw: zragf=%lu zlib=%lu delta=%ld ms_zragf=%d ms_zlib=%d\n",
               cases[i],
    ...
    {
        size_t n = 0u, emitted = 0u;
        unsigned char *buf = make_case("jsonish", &n);
        zragf_fx ms_rebuild, ms_prepared;
        if (!buf) return 1;
        ms_rebuild = bench_internal_dynamic(buf, n, 0, &emitted);
        ms_prepared = bench_internal_dynamic(buf, n, 1, &emitted);
        printf("phase31 internal jsonish dynamic: rebuild=%d prepared=%d size=%lu\n",
               ms_rebuild, ms_prepared, (unsigned long)emitted);
        zragf_p89_host_release(buf);
    }
