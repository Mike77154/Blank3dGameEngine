...
def load_cases(root: pathlib.Path) -> List[Dict[str, Any]]:
    ...
    if generated_manifest.exists():
        ...
        for case in data.get("cases", []):
            ...
            cases.append(case)
    external_manifest = root / "tests" / "external_corpus" / "bmp_manifest.json"
...
def eval_case(cli: pathlib.Path, case: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if proc.stdout.strip():
        ...
        result["observed"] = {"parse_ok": False, "decode_ok": False, "parse_status": "NO_OUTPUT", "decode_status": "NO_OUTPUT"}
...
def render_md(results: List[Dict[str, Any]]) -> str:
    lines = ["# BMP compatibility matrix", "", "| Case | Origin | Parse | Decode | Expectation |", "|---|---|---:|---:|---:|"]
    for item in results:
        if item.get("status") == "missing":
            lines.append(f"| {item['id']} | {item['origin']} | n/a | n/a | missing |")
            continue
        obs = item["observed"]
        lines.append(
            f"| {item['id']} | {item['origin']} | {obs.get('parse_ok')} | {obs.get('decode_ok')} | {item.get('matches_expectation')} |"
        )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument("--cli", default="tools/bmp_compat_cli")
