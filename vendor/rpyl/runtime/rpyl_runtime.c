...
static void rt_log0(const char* msg) {
#if RPYL_ENABLE_LOG_STDIO
...
static void rt_log_s(const char* prefix, const char* value) {
#if RPYL_ENABLE_LOG_STDIO
    if (prefix) fputs(prefix, stderr);
    fputs(value ? value : "(null)", stderr);
    fputc('\n', stderr);
#else
    (void)prefix;
    (void)value;
...
static void rt_log_i(const char* prefix, int value) {
#if RPYL_ENABLE_LOG_STDIO
    if (prefix) fputs(prefix, stderr);
    fprintf(stderr, "%d\n", value);
#else
...
static void rt_log_ul(const char* prefix, unsigned long value) {
#if RPYL_ENABLE_LOG_STDIO
    if (prefix) fputs(prefix, stderr);
    fprintf(stderr, "%lu\n", value);
#else
...
static RpylRuntime g_runtime_pool[RPYL_MAX_RUNTIMES];
...
static const char* resolve_token(RpylRuntime* rt, const char* token) {
    const char* v;

    if (!token) return "";
    if (token[0] != '$' || token[1] == 0) return token;

    v = rpyl_runtime_get_var(rt, token);
    if (v) return v;
...
static void build_joined_name(RpylRuntime* rt, const char** parts, int count, char* out, size_t out_size) {
    ...
    for (i = 0; i < count; i++) {
        ...
        s = resolve_token(rt, parts ? parts[i] : "");
        sl = strlen(s);
...
static void dispatch_external(RpylRuntime* rt, const char* name, const char** args, int argc) {
    RpylCmdFn fn;
    const char* resolved_args[RPYL_RUNTIME_MAX_ARGS];
    int resolved_argc;
    int i;
    ...
    resolved_argc = argc;
    if (resolved_argc < 0) resolved_argc = 0;
    if (resolved_argc > (int)(sizeof(resolved_args) / sizeof(resolved_args[0]))) {
        resolved_argc = (int)(sizeof(resolved_args) / sizeof(resolved_args[0]));
    }
    for (i = 0; i < resolved_argc; i++) {
        resolved_args[i] = resolve_token(rt, args ? args[i] : "");
    }
    ...
    */
    if (strcmp(name, "yield") == 0 || strcmp(name, "pause") == 0) {
        ...
        if (resolved_argc >= 1) token = parse_ul_token(resolved_args[0]);
    ...
    if (!fn) {
        rt_log_s("[Rpyl] Command not found: ", name);
        return;
    }

    fn(rt, name, resolved_args, resolved_argc);
}
...
static void runtime_init(RpylRuntime* rt) {
    int keep_active;
    if (!rt) return;
    keep_active = rt->active;
    memset(rt, 0, sizeof(*rt));
    rt->active = keep_active;
    reset_scopes(rt);
...
RpylRuntime* rpyl_runtime_create(void) {
    ...
    for (i = 0; i < RPYL_MAX_RUNTIMES; i++) {
        if (!g_runtime_pool[i].active) {
            rt = &g_runtime_pool[i];
            memset(rt, 0, sizeof(*rt));
            rt->active = 1;
            runtime_init(rt);
...
int rpyl_runtime_register(RpylRuntime* rt, const char* command, RpylCmdFn fn) {
    ...
    if (rt->command_count >= (int)(sizeof(rt->commands) / sizeof(rt->commands[0]))) {
        rt_log_i("[Rpyl] Too many runtime commands (max ", (int)(sizeof(rt->commands) / sizeof(rt->commands[0])));
        return 0;
    }
...
void rpyl_runtime_clear_defines(RpylRuntime* rt) {
    if (!rt) return;
    rt->define_count = 0;
...
static int exec_command(
    ...
    if (strcmp(name, "call") == 0) {
        ...
        if (argc < 1) {
            rt_log0("[Rpyl] call: missing target label\n");
            *stmt = next_stmt;
            return 1;
        }
        ...
        if (!target) {
            rt_log_s("[Rpyl] call: label not found: ", label);
            *stmt = next_stmt;
            return 1;
        }

        if (*sp >= RPYL_RUNTIME_MAX_STACK) {
            rt_log0("[Rpyl] call stack overflow\n");
            *stmt = next_stmt;
            return 1;
        }
    ...
    if (strcmp(name, "jump") == 0 || strcmp(name, "goto") == 0) {
        ...
        if (argc < 1) {
            rt_log0("[Rpyl] jump: missing target label\n");
            *stmt = next_stmt;
            return 1;
        }
        ...
        target = find_block(root, label);
        if (!target) {
            rt_log_s("[Rpyl] jump: label not found: ", label);
            *stmt = next_stmt;
            return 1;
        }
...
int rpyl_runtime_execute(
    ...
    if (!current_block) {
        rt_log_s("[Rpyl] Block not found: ", block_name);
        return 0;
    }
...

        {
            ...
            if (node->type == AST_BLOCK) {
                ...
                if (sp >= (int)(sizeof(stack) / sizeof(stack[0]))) {
                    rt_log0("[Rpyl] nested stack overflow\n");
                    stmt = next_stmt;
                    continue;
                }

                /* Enter nested block: push nested frame */
...
static int bc_read_string_args(const RpylBytecode* bc, rpyl_u32* ip, rpyl_u32 raw_argc, const char** out, int out_cap, int* out_count) {
    rpyl_u32 i;
    int used;

    if (out_count) *out_count = 0;
    if (!bc || !ip || !out || out_cap < 0) return 0;
    ...
    for (i = 0; i < raw_argc; i++) {
        ...
        (*ip)++;
        if (used < out_cap) {
            if ((size_t)sid >= bc->string_count) out[used] = "";
            else out[used] = bc->strings[sid] ? bc->strings[sid] : "";
            used++;
        }
    }
...
int rpyl_runtime_execute_bytecode(
    ...
    if (label_index < 0) {
        rt_log_s("[Rpyl] Bytecode entry label not found: ", entry_label);
        return 0;
    }
    ...
    if (ip > (rpyl_u32)bc->code_count || end_ip > (rpyl_u32)bc->code_count || ip > end_ip) {
        rt_log_s("[Rpyl] Bytecode label range invalid: ", entry_label);
        return 0;
    }
    ...
    while (1) {
...

        {
            ...
            if (op == (rpyl_u32)RPYL_BC_OP_CMD) {
                ...
                if (!bc_read_string_args(bc, &ip, argc, argv, (int)(sizeof(argv) / sizeof(argv[0])), &argc_read)) return 0;

                dispatch_external(rt, bc->strings[name_sid], argv, argc_read);
                continue;
            }
            ...
            if (op == (rpyl_u32)RPYL_BC_OP_SET) {
                ...
                var_name = bc->strings[var_sid];
                if (var_name[0] == '$') var_name++;

                if (!bc_read_string_args(bc, &ip, argc, argv, (int)(sizeof(argv) / sizeof(argv[0])), &argc_read)) return 0;

                build_joined_name(rt, argv, argc_read, joined, sizeof(joined));
            ...
            if (op == (rpyl_u32)RPYL_BC_OP_CALL || op == (rpyl_u32)RPYL_BC_OP_JUMP) {
                ...
                build_joined_name(rt, argv, argc_read, target_name, sizeof(target_name));
                tgt_idx = bc_label_find(bc, target_name);
                if (tgt_idx < 0) {
                    rt_log_s("[Rpyl] Unknown label: ", target_name);
                    continue;
                }
            ...
            rt_log_ul("[Rpyl] Unknown bytecode opcode: ", (unsigned long)op);
            return 0;
        }
    }
...
int rpyl_runtime_begin_ast(RpylRuntime* rt, AstNode* root, const char* entry_block) {
    ...
    if (!current_block) {
        rt_log_s("[Rpyl] Block not found: ", entry_block);
        return 0;
    }
    ...
    while (1) {
...

        {
            ...
            if (op == (rpyl_u32)RPYL_BC_OP_CMD) {
                ...
                if (!bc_read_string_args(bc, &rt->bc_ip, argc, argv, (int)(sizeof(argv) / sizeof(argv[0])), &argc_read)) return 0;
