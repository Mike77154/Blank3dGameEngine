...
ProjectileVisual2D89 is the 2D-in-3D projectile presentation authority. It requests the current sprite frame through a provider and emits renderer-neutral billboard geometry, layers, mesh-replacement policy and light samples. Asset authoring (static/sequence/RenList/tilecell/GameMaker strip) remains behind the frame provider, so the WeaponSystem does not depend on any decoder or renderer.
