...
int gsw89_profile_from_recipe(const gri89_doc *doc, gsw89_profile *out_profile)
{
    gsw89_profile p;
    if (!doc || !out_profile) return 0;
    p.yaw_amp_x1000 = (short)gri89_get_long(doc, "sway", "yaw_amp_x1000", 110);
    p.pitch_amp_x1000 = (short)gri89_get_long(doc, "sway", "pitch_amp_x1000", 90);
    p.drift_cycle_frames = (short)gri89_get_long(doc, "sway", "drift_cycle_frames", 115);
    p.breath_amp_x1000 = (short)gri89_get_long(doc, "sway", "breath_amp_x1000", 75);
    p.breath_cycle_frames = (short)gri89_get_long(doc, "sway", "breath_cycle_frames", 210);
    p.pulse_amp_x1000 = (short)gri89_get_long(doc, "sway", "pulse_amp_x1000", 12);
    p.pulse_cycle_frames = (short)gri89_get_long(doc, "sway", "pulse_cycle_frames", 52);
    p.hold_reduction_pct = (short)gri89_get_long(doc, "sway", "hold_reduction_pct", 88);
    p.hold_enter_frames = (short)gri89_get_long(doc, "sway", "hold_enter_frames", 12);
    p.hold_max_frames = (short)gri89_get_long(doc, "sway", "hold_max_frames", 180);
    p.recovery_frames = (short)gri89_get_long(doc, "sway", "recovery_frames", 150);
    p.movement_gain_pct = (short)gri89_get_long(doc, "sway", "movement_gain_pct", 140);
    p.stress_gain_pct = (short)gri89_get_long(doc, "sway", "stress_gain_pct", 70);
    *out_profile = p;
    return 1;
}

int gsw89_profile_load_ini(const char *path, gsw89_profile *out_profile)
{
    gri89_doc doc;
    if (!path || !out_profile) return 0;
    gri89_init(&doc);
    if (!gri89_load(&doc, path)) return 0;
    return gsw89_profile_from_recipe(&doc, out_profile);
}
