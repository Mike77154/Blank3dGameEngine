...
int gsh89_profile_from_recipe(const gri89_doc *doc,
                              gsh89_profile *out_profile)
{
    ...
    p.overlay_id = (short)gri89_get_long(doc, "hud", "overlay_id", 3);
    p.hud_id = (short)gri89_get_long(doc, "hud", "hud_id", 3);
    flags = 0;
    if (gri89_get_bool(doc, "mask", "blackout", 0))
        flags = (short)(flags | GSH89_HUD_BLACKOUT);
    if (gri89_get_bool(doc, "reticle", "crosshair", 0))
        flags = (short)(flags | GSH89_HUD_CROSSHAIR);
    if (gri89_get_bool(doc, "reticle", "center_dot", 0))
        flags = (short)(flags | GSH89_HUD_CENTER_DOT);
    if (gri89_get_bool(doc, "reticle", "range_ticks", 0))
        flags = (short)(flags | GSH89_HUD_RANGE_TICKS);
    if (gri89_get_bool(doc, "mask", "vignette", 0))
        flags = (short)(flags | GSH89_HUD_VIGNETTE);
    if (gri89_get_bool(doc, "telemetry", "optic", 0))
        flags = (short)(flags | GSH89_HUD_OPTIC_DATA);
    if (gri89_get_bool(doc, "telemetry", "target", 0))
        flags = (short)(flags | GSH89_HUD_TARGET_DATA);
    if (gri89_get_bool(doc, "telemetry", "breath", 0))
        flags = (short)(flags | GSH89_HUD_BREATH_DATA);
    if (gri89_get_bool(doc, "telemetry", "impact", 0))
        flags = (short)(flags | GSH89_HUD_IMPACT_DATA);
    p.flags = flags;

    p.reticle_gap_px = (short)gri89_get_long(doc, "reticle", "gap_px", 8);
    p.reticle_arm_px = (short)gri89_get_long(doc, "reticle", "arm_px", 70);
    p.reticle_thickness = (short)gri89_get_long(doc, "reticle", "thickness_px", 1);
    p.tick_count = (short)gri89_get_long(doc, "reticle", "tick_count", 4);
    p.tick_spacing_px = (short)gri89_get_long(doc, "reticle", "tick_spacing_px", 24);
    p.mask_alpha = (short)gri89_get_long(doc, "mask", "alpha", 255);
    p.vignette_alpha = (short)gri89_get_long(doc, "mask", "vignette_alpha", 180);
    *out_profile = p;
    return 1;
}

int gsh89_profile_load_ini(const char *path,
                           gsh89_profile *out_profile)
{
    gri89_doc doc;
    ...
    return gsh89_profile_from_recipe(&doc, out_profile);
}
