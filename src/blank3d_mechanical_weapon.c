...
#define B3D_MECH89_RIG_TAG 3919
                                   ...
                                   nm89_matrix *out_world)
{
    ...
    (void)part_id;
    (void)source_id;
    weapon = (Blank3DMechanicalWeapon *)user;
                                       ...
                                       nm89_transform_sample *out_sample)
{
    ...
    if (weapon == 0 || out_sample == 0 ||
        source_id != B3D_MECH89_PART_MAGAZINE)
        return NM89_ERR_PROVIDER;
                                      ...
                                      const nm89_geometry_packet *packet)
{
    ...
    if (weapon->packet_count >= B3D_MECH89_MAX_PACKETS) return;
...
static int b3d_mech89_add_binding(Blank3DMechanicalWeapon *weapon,
                                  nm89_i16 part_id,
                                  int mesh_id)
{
    int model_id;
...
static int b3d_mech89_build_fire_clip(Blank3DMechanicalWeapon *weapon)
{
    nm89_i16 track;
    ...
    result = nm89_clip_begin(&weapon->rig, B3D_MECH89_ACTION_FIRE,
    ...
    if (weapon->presentation.mechanism_kind == B3D_MECHANISM_REVOLVER) {
        ...
        if (nm89_track_add_key(&weapon->rig, track, 0U, 0) != NM89_OK ||
            ...
            return NM89_ERR_CAPACITY;
    } else if (weapon->presentation.mechanism_kind != B3D_MECHANISM_FIXED &&
               weapon->presentation.mechanism_kind != B3D_MECHANISM_ELASTIC) {
        result = nm89_clip_add_track(&weapon->rig, weapon->slide_part,
                                     NM89_PROP_MOVE_Z,
                                     NM89_INTERP_SMOOTH, &track);
        if (result != NM89_OK) return result;
    ...
    event_value.code = B3D_MECH89_EVENT_SLIDE_REAR;
    ...
    const Blank3DWeaponPresentation *presentation)
{
    ...
    Blank3DWeaponPresentation fallback;

    if (weapon == 0) return 0;
    if (presentation == 0) {
        blank3d_weapon_presentation_defaults(&fallback, 1, "weapon");
    ...
    nm89_rig_init(&weapon->rig, B3D_MECH89_RIG_TAG);
...
int blank3d_mechanical_weapon_init(Blank3DMechanicalWeapon *weapon)
{
    ...
    blank3d_weapon_presentation_defaults(&presentation, 1, "weapon");
    return blank3d_mechanical_weapon_init_profile(weapon, &presentation);
}
    ...
    unsigned short frame_ms)
{
    ...
    delta_ticks = (nm89_u16)(total_ms / B3D_MECH89_TICK_MS);
    weapon->tick_remainder_ms = total_ms % B3D_MECH89_TICK_MS;
    weapon->packet_count = 0;
...
const nm89_pose *blank3d_mechanical_weapon_muzzle_socket(
    const Blank3DMechanicalWeapon *weapon)
{
    return blank3d_mechanical_weapon_part_pose(
        weapon, B3D_MECH89_PART_MUZZLE_SOCKET);
}
