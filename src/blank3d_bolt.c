...
static int blank3d_bolt_trace(void *user_data,
                              ...
                              B3D_Hit *out_hit)
{
    ...
    bolt = (Blank3DBolt *)user_data;
    if (!bolt || !bolt->collision || !request || !out_hit) return 0;
    ...
    if (!blank3d_collision_sweep_bullet(bolt->collision, &start, &delta,
                                       q16_to_q12(request->radius), &hit))
        return 0;

    b3d_hit_clear(out_hit);
    out_hit->hit = 1;
    out_hit->hit_kind = hit.enemy_index >= 0
                      ? B3D_HIT_ENTITY : B3D_HIT_WORLD;
    out_hit->target_id = hit.enemy_index;
    out_hit->target_slot = hit.enemy_index;
    out_hit->target_layer = hit.enemy_index >= 0
                          ? B3D_LAYER_ENEMY : B3D_LAYER_WORLD;
...
void blank3d_bolt_init(Blank3DBolt *bolt, Blank3DCollision *collision)
{
    ...
    bolt->collision = collision;
    b3d_world_init(&bolt->world,
                   bolt->projectiles, BLANK3D_BOLT_MAX_PROJECTILES,
                   bolt->events, BLANK3D_BOLT_MAX_EVENTS,
                   bolt->colliders, BLANK3D_BOLT_MAX_COLLIDERS);
    b3d_callbacks_clear(&callbacks);
    callbacks.user_data = bolt;
    callbacks.trace_world = blank3d_bolt_trace;
    b3d_world_set_callbacks(&bolt->world, &callbacks);
    for (i = 0; i < BLANK3D_BOLT_MAX_PROJECTILES; ++i)
        bolt->drag_q16[i] = 0;
}
                       ...
                       void *user_ptr)
{
    ...
    if (projectile && projectile->slot >= 0 &&
        projectile->slot < BLANK3D_BOLT_MAX_PROJECTILES)
