...
{
    static const int kx[B3D_SHOTGUN_PELLET_COUNT] = {
        0, -1000, 1000, -650, 650, -300, 300
    };
    static const int ky[B3D_SHOTGUN_PELLET_COUNT] = {
    ...
    if (!source || !camera_origin || !camera_forward ||
        ...
        return 0;
    if (source->weapon_id != B3D_SHOTGUN_WEAPON_ID) return 0;

    *out_event = *source;
    index = source->pellet_index;
    if (index < 0) index = 0;
    index %= B3D_SHOTGUN_PELLET_COUNT;
    ...
    out_event->pellet_count = B3D_SHOTGUN_PELLET_COUNT;
    ...
    if (out_event->life_ms < B3D_SHOTGUN_MIN_LIFE_MS)
        out_event->life_ms = B3D_SHOTGUN_MIN_LIFE_MS;
